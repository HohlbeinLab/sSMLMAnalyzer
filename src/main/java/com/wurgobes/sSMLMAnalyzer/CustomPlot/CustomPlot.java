...
import com.wurgobes.sSMLMAnalyzer.OwnColorTable;
import ij.*;
import ij.gui.*;
import ij.process.*;
import ij.util.*;
...
public class CustomPlot extends Plot implements Cloneable {
	...
	int preferredPlotWidth = CustomPlotWindow.plotWidth;  //default size of plot frame (not taking 'High-Resolution' scale factor into account)
	int preferredPlotHeight = CustomPlotWindow.plotHeight;
	...
	public CustomPlot(String title, String xLabel, String yLabel, LUTService ls, String defaultLUT) {
		this(title, xLabel, yLabel, null, null, getDefaultFlags(), ls, defaultLUT);
	}

	public CustomPlot(String title, String xLabel, String yLabel, float[] xValues, float[] yValues, Object object) {
		this(title, xLabel, yLabel, xValues, yValues, getDefaultFlags(), null, "");
	}
	...
	public CustomPlot(String title, String xLabel, String yLabel, float[] xValues, float[] yValues, int flags, LUTService ls, String defaultLUT) {
		...
		pp.axisFlags = getDefaultFlags();
		if(this.ownColorTable == null && ls != null) {
			ownColorTable = new OwnColorTable(ls);
	...
	public void makeAreaHist(Roi roi){
		...
		for(int i = 0; i < mainPlotObject.xValues.length; i++){
			if(roi.contains(scaleX(mainPlotObject.xValues[i]), scaleY(mainPlotObject.yValues[i])))
				values.add(mainPlotObject.zValues[i]);
		}

		String title = "selection " + (histwindows.size() + 1);
	...
	void setLimitsNoUpdate(double xMin, double xMax, double yMin, double yMax) {
		...
		if (containsNaN && getNumPlotObjects(PlotObject.XYZ_DATA | PlotObject.XY_DATA| PlotObject.ARROWS, false)==0)//can't apply auto-range without data
			return;
	...
	/** Sets options for the CustomPlot. Multiple options may be separated by whitespace or commas.
	...
	*/
	 ...
	 *  (usually default) templateFlags of this CustomPlot.
	 *  <code>plot</code> may be null; then the call has no effect. */
	public void useTemplate(CustomPlot plot) {
	 ...
	 *	<code>plot</code> may be null; then the call has no effect. */
	public void useTemplate(CustomPlot plot, int templateFlags) {
		if (plot == null) return;
	 ...
	 *	Should be called before creating the CustomPlot.
	...
	public void setScale(float scale) {
		...
		this.pp.width = sc(pp.width);
		this.pp.height = sc(pp.height);
		plotDrawn = false;
	}
	 ...
	 *	Call update() thereafter to make the change visible (if the image is shown already). */
	public void setTickLength(int t) {
			tickLength = t;
	}

	/** Sets the length of the minor tick in pixels. */
	public void setMinorTickLength(int t) {
			minorTickLength = t;
	}
	...
	*/
	...
	public static int toShape(String str) {
		...
		if (str.contains("curve") || str.contains("line"))
			...
			shape = CustomPlot.TRIANGLE;
		else if (str.contains("cross") || str.contains("+"))
			shape = CustomPlot.CROSS;
		else if (str.contains("diamond"))
			shape = CustomPlot.DIAMOND;
		else if (str.contains("dot"))
			shape = CustomPlot.DOT;
	...
	public void setLegend(String labels, int flags) {
		if (labels != null && labels.length()>0) {
			...
			int iPart = 0;
			for (PlotObject plotObject : allPlotObjects)
				if ((plotObject.type == PlotObject.XYZ_DATA || plotObject.type == PlotObject.XY_DATA) && !plotObject.hasFlag(PlotObject.HIDDEN))
	 ...
	 * is CustomPlot.LEFT, CustomPlot.CENTER or CustomPlot.RIGHT. Default is LEFT. */
	public void setJustification(int justification) {
		currentJustification = justification;
	}

	/** Changes the drawing color for the next objects that will be added to the CustomPlot.
	...
	/** Sets the drawing colors for the next objects that will be added to the CustomPlot. */
	...
	/** Changes the line width for the next objects that will be added to the CustomPlot. */
	...
	**/
	public float[][] getDataObjectArrays(int index) {
		int i = 0;
		for (PlotObject plotObject : allPlotObjects) {
			if ( (plotObject.type != PlotObject.XYZ_DATA && plotObject.type != PlotObject.XY_DATA )|| plotObject.hasFlag(PlotObject.HIDDEN)) continue;
			if (index == i)
	...
	public int addObjectFromPlot(CustomPlot plot, int i) {
	...
	*/
	...
	public int getPlotObjectIndex(float[][] values) {
		return getPlotObjectIndex(PlotObject.XY_DATA|PlotObject.ARROWS|PlotObject.XYZ_DATA, values);
	}
	...
	/** Draws the plot (if not done before) in an ImageProcessor and returns the ImageProcessor with the CustomPlot. */
	 ...
	 *  Does nothing if imp is unchanged and has the ImageProcessor of this CustomPlot.
	...
	public CustomPlotWindow	showCustom() {
		...
		if ((IJ.macroRunning() && IJ.getInstance()==null) || Interpreter.isBatchMode()) {
			...
			if (getMainCurveObject() != null) {
				...
				imp.setProperty("YValues", getYValues()); // by CustomPlot.getValues() macro function
			}
		...
		if (imp != null) {
			...
			if (win instanceof CustomPlotWindow && win.isVisible()) {
				updateImage();			// show in existing window
				return (CustomPlotWindow)win;
			} else
				setImagePlus(null);
		}
		CustomPlotWindow pw = new CustomPlotWindow(this);		//note: this may set imp to null if pw has listValues and autoClose are set
	...
	/** Releases the ImageProcessor and ImagePlus associated with the CustomPlot.
	...
	/** Scales a value of the original plot for a high-resolution CustomPlot. Returns an integer number of pixels >=1 */
	int sc(float length) {
	...
	/** Scales a font of the original plot for a high-resolution CustomPlot. */
	...
	double[] getMinAndMax(boolean allObjects, int axisRangeFlags) {
		...
		for (PlotObject plotObject : allPlotObjects) {
			if ((plotObject.type == PlotObject.XYZ_DATA || plotObject.type == PlotObject.XY_DATA || plotObject.type == PlotObject.ARROWS) && !plotObject.hasFlag(PlotObject.HIDDEN)) {
	 ...
	 *  Does not set the new limits as default, does not redraw the CustomPlot. */
	...
	*/
			...
			if (xMin==xMax) {
				...
				for (int i=0; i<=(i2-i1); i++) {
					...
					if (xCats!= null) {
						...
						if(index >= 0 && index < xCats.length  && remainder < 1e-9){
							...
							int w = 0;
							for (String part : parts) w = Math.max(w, ip.getStringWidth(part));
				...
				if (minorTicks && (!logXAxis || step > 1.1)) {  //'standard' log minor ticks only for full decades
					...
					for (int i=i1; i<=i2; i++) {
						double v = i*mstep;
						int x = (int)Math.round((v - xMin)*xScale) + leftMargin;
						drawLine(x, y1, x, y1+sc(minorTickLength));
						drawLine(x, y2, x, y2-sc(minorTickLength));
					}
		...
		if (hasFlag(Y_NUMBERS | (logYAxis ? (Y_TICKS | Y_MINOR_TICKS) : Y_LOG_TICKS) + Y_GRID)) {
			...
			if (yMin==yMax) {
				...
				for (int i=i1; i<=i2; i++) {
					...
					if (majorTicks) {
						drawLine(x1, y, x1+sc(tickLength), y);
				...
				if (minorTicks && (!logYAxis || step > 1.1)) {  //'standard' log minor ticks only for full decades
					...
					for (int i=i1; i<=i2; i++) {
						double v = i*mstep;
						int y = topMargin + frameHeight - (int)Math.round((v - yMin)*yScale);
						drawLine(x1, y, x1+sc(minorTickLength), y);
						drawLine(x2, y, x2-sc(minorTickLength), y);
					}
				}
	...
	static int getDigits2(double n, double resolution, int maxDigits, int suggestedDigits) {
		...
		if ((digits < -2 && log10ofN >= maxDigits) || suggestedDigits < 0)
			digits = sciDigits; //scientific notation for large numbers or if desired via suggestedDigits (CustomPlot.setOptions)
		else if (digits < 0)
			digits = 0;
		...
		switch (type) {
			...
			case PlotObject.XY_DATA:
				...
				if (plotObject.hasFilledMarker()) { // fill markers with secondary color
					...
					if(type != PlotObject.XYZ_DATA)  ip.setColor(plotObject.color2);
					ip.setLineWidth(1);
				...
				if (plotObject.hasMarker()) {       // draw the marker symbols
					int markSize = plotObject.getMarkerSize();
					if(type != PlotObject.XYZ_DATA) ip.setColor(plotObject.color);
		...
		for (PlotObject plotObject : allPlotObjects)
			if (plotObject.type == PlotObject.XY_DATA && !plotObject.hasFlag(PlotObject.HIDDEN)) {
				if (first)
					...
					labels.append('\n');
				if (plotObject.label != null) labels.append(plotObject.label);
			}
	...
	/** Creates a ResultsTable with the data of the CustomPlot. Returns null if no data.
	...
	static int getPrecision(float[] values) {
		...
		for (float value : values) {
			if ((int) value != value && !Float.isNaN(value)) {
				allInteger = false;
...
class PlotProperties implements Cloneable, Serializable {
	...
	//
	PlotObject frame = new PlotObject(CustomPlot.DEFAULT_FRAME_LINE_WIDTH);	 //the frame, including background color and axis numbering
...
class PlotObject implements Cloneable, Serializable {
	...
	public int flags;
	/**  Options, currently only for FRAME, see CustomPlot.setOptions */
	...
	public float[] xValues, yValues, xEValues, yEValues, zValues;
	...
	/** Type of the points, such as CustomPlot.LINE, CustomPlot.CROSS etc. (for type = XY_DATA) */
	public int shape;
	...
	public String label;
	/** Labels only: Justification can be CustomPlot.LEFT, CustomPlot.CENTER or CustomPlot.RIGHT */
	...
	PlotObject(float[] xValues, float[] yValues, float[] yErrorBars, int shape, float lineWidth, Color color, Color color2, String yLabel) {
		...
		this.color2 = color2;
		this.label = yLabel;
		if (shape==CustomPlot.CUSTOM)
			this.macroCode = yLabel;
	}

	/** Constructor for XYZ_DATA, i.e., a curve or set of points */
	PlotObject(float[] xValues, float[] yValues, float[] zValues, float[] yErrorBars, int shape, float lineWidth, String yLabel) {
		this.type = XYZ_DATA;
		this.xValues = xValues;
		this.yValues = yValues;
		this.yEValues = yErrorBars;
	...
	boolean hasCurve() {
		return type == XY_DATA && (shape == CustomPlot.LINE || shape == CustomPlot.CONNECTED_CIRCLES || shape == CustomPlot.FILLED);
	}

	/** Whether an XY_DATA object has markers to draw */
	boolean hasMarker() {
		return (type == XY_DATA || type == XYZ_DATA) && (shape == CustomPlot.CIRCLE || shape == CustomPlot.X || shape == CustomPlot.BOX || shape == CustomPlot.TRIANGLE
				|| shape == CustomPlot.CROSS || shape == CustomPlot.DIAMOND || shape == CustomPlot.DOT || shape == CustomPlot.CONNECTED_CIRCLES
				|| shape == CustomPlot.CUSTOM);
	}

	/** Whether an XY_DATA object has markers that can be filled */
	boolean hasFilledMarker() {
		return type == XY_DATA && color2 != null && (shape == CustomPlot.CIRCLE || shape == CustomPlot.BOX || shape == CustomPlot.TRIANGLE ||
				shape == CustomPlot.DIAMOND || shape == CustomPlot.CONNECTED_CIRCLES);
	}
	...
	private ArrayList<float[]> cloneArrayList(ArrayList<float[]> src) {
		ArrayList<float[]> dest = (ArrayList<float[]>) src.clone();     //shallow clone
		...
		for (int i=0; i<dest.size(); i++) {
			...
			if (o != null) try {
				...
				dest.set(i, (float[]) cloneMethod.invoke(o, noObjects));
