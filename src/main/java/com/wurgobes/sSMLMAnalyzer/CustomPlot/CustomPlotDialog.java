...
public class CustomPlotDialog implements DialogListener {
    ...
    public static final int SET_RANGE = 0, AXIS_OPTIONS = 1, LEGEND = 2, HI_RESOLUTION = 3, TEMPLATE = 4, //5-9 spare
            X_LEFT = 10, X_RIGHT = 11, Y_BOTTOM = 12, Y_TOP = 13, X_AXIS = 14, Y_AXIS = 15, LUT_CHANGE = 16;
    ...
    private static final String[] HEADINGS = new String[] {"Plot Range", "Axis Options", "Add Legend", "High-Resolution Plot", "Use Template",
            ...
            "X Left", "X Right", "Y Bottom","Y Top", "X Axis", "Y Axis", "Change LUT"};
    ...
    private static final int[] LEGEND_POSITION_N = new int[] {CustomPlot.AUTO_POSITION, CustomPlot.TOP_LEFT, CustomPlot.TOP_RIGHT, CustomPlot.BOTTOM_LEFT, CustomPlot.BOTTOM_RIGHT, 0};
    ...
    private static final int[] TEMPLATE_FLAGS = new int[] {CustomPlot.X_RANGE, CustomPlot.Y_RANGE, CustomPlot.COPY_AXIS_STYLE, CustomPlot.COPY_LABELS,
            CustomPlot.COPY_LEGEND, CustomPlot.COPY_CONTENTS_STYLE, CustomPlot.COPY_EXTRA_OBJECTS, CustomPlot.COPY_SIZE};
    ...
    private CustomPlot[]  templatePlots;
    ...
    public CustomPlotDialog(CustomPlot plot, int dialogType, OwnColorTable ct) {
        this.plot = plot;
        this.dialogType = dialogType;
        this.ownColorTable = ct;
        this.colors = ownColorTable.getLuts();
        Arrays.sort(colors);
    }
    ...
    public void showDialog(Frame parent) {
        ...
        if (win instanceof PlotWindow)
            ((CustomPlotWindow)win).hideRangeArrows(); // arrows etc might be still visible, but the mouse maybe elsewhere
        ...
        if (!gd.wasCanceled() && !gd.wasOKed()) { // user has pressed "Set all limits" or "Set Axis Options" button
            ...
            new ij.gui.PlotDialog(plot, newDialogType).showDialog(parent);
        }
    }
    ...
    private boolean setupDialog(GenericDialog gd) {
        ...
        if (dialogType == SET_RANGE || dialogType == X_AXIS) {
            ...
            if (livePlot)
                gd.addCheckbox("Fix_X Range While Live", (plot.templateFlags & CustomPlot.X_RANGE) != 0);
            gd.addCheckbox("Log_X Axis  **", (plot.hasFlag(CustomPlot.X_LOG_NUMBERS)));
        ...
        if (dialogType == SET_RANGE || dialogType == Y_AXIS) {
            ...
            if (livePlot)
                gd.addCheckbox("Fix_Y Range While Live", (plot.templateFlags & CustomPlot.Y_RANGE) != 0);
            gd.addCheckbox("Log_Y Axis  **", (plot.hasFlag(CustomPlot.Y_LOG_NUMBERS)));
        ...
        if (dialogType == AXIS_OPTIONS || dialogType == X_AXIS || dialogType == Y_AXIS) {
            ...
            final int[] xFlags = new int[] {CustomPlot.X_GRID, CustomPlot.X_TICKS, CustomPlot.X_MINOR_TICKS, CustomPlot.X_LOG_TICKS, CustomPlot.X_NUMBERS};
        ...
        if (dialogType == LEGEND) {
            ...
            if (lFlags != -1) { //if we have a legend already
                ...
                transparentBackground = getFlag(lFlags, CustomPlot.LEGEND_TRANSPARENT);
                bottomUp = getFlag(lFlags, CustomPlot.LEGEND_BOTTOM_UP);
            }
        ...
        if (dialogType == TEMPLATE) {
            ...
            templatePlots = new CustomPlot[idList.length];
            int nPlots = 0;
            for (int id : idList) {
                ImagePlus imp = WindowManager.getImage(id);
                if (imp == null) continue;
                CustomPlot impPlot = (CustomPlot)(imp.getProperty(CustomPlot.PROPERTY_KEY));
            ...
            if (lastTemplateFlags == 0) lastTemplateFlags = CustomPlot.COPY_AXIS_STYLE|CustomPlot.COPY_CONTENTS_STYLE;
    ...
    public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
        ...
        if (dialogType == SET_RANGE || dialogType == X_AXIS) {
            ...
            if (livePlot)
                plot.templateFlags = setFlag(plot.templateFlags, CustomPlot.X_RANGE, gd.getNextBoolean());
        ...
        if (dialogType == SET_RANGE || dialogType == Y_AXIS) {
            ...
            if (livePlot)
                plot.templateFlags = setFlag(plot.templateFlags, CustomPlot.Y_RANGE, gd.getNextBoolean());
        ...
        if (dialogType == LEGEND) {
            ...
            if (bottomUp)
                lFlags |= CustomPlot.LEGEND_BOTTOM_UP;
            if (transparentBackground)
                lFlags |= CustomPlot.LEGEND_TRANSPARENT;
    ...
    private void record() {
        if (Recorder.scriptMode())
            Recorder.recordCall("//plot = IJ.getImage().getProperty(CustomPlot.PROPERTY_KEY);");
        String plotDot = Recorder.scriptMode() ? "plot." : "CustomPlot.";

        if (dialogType == SET_RANGE || dialogType == X_AXIS)
            Recorder.recordString(plotDot+(Recorder.scriptMode() ? "setAxisXLog(" : "setLogScaleX(")+plot.hasFlag(CustomPlot.X_LOG_NUMBERS)+");\n");
        if (dialogType == SET_RANGE || dialogType == Y_AXIS)
            Recorder.recordString(plotDot+(Recorder.scriptMode() ? "setAxisYLog(" : "setLogScaleY(")+plot.hasFlag(CustomPlot.Y_LOG_NUMBERS)+");\n");
        if (dialogType == SET_RANGE || dialogType == X_AXIS || dialogType == Y_AXIS) {
            double[] currentMinMax = plot.getLimits();
            int xDigits = plot.logXAxis ? -2 : CustomPlot.getDigits(currentMinMax[0], currentMinMax[1], 0.005*Math.abs(currentMinMax[1]-currentMinMax[0]), 6, 0);
            int yDigits = plot.logYAxis ? -2 : CustomPlot.getDigits(currentMinMax[2], currentMinMax[3], 0.005*Math.abs(currentMinMax[3]-currentMinMax[2]), 6, 0);
        ...
        if (dialogType == AXIS_OPTIONS || dialogType == X_AXIS || dialogType == Y_AXIS) {
            ...
            if (labelFont != null) {
                if (Recorder.scriptMode())
                    ...
                    Recorder.recordString("CustomPlot.setAxisLabelSize("+IJ.d2s(labelFont.getSize2D(),1)+", \""+(labelFont.isBold() ? "bold" : "plain")+"\");\n");
            }
            if (numberFont != null)
        ...
        if (dialogType == LEGEND) {
            ...
            if (Recorder.scriptMode()) {
                ...
                if (getFlag(lFlags, CustomPlot.LEGEND_BOTTOM_UP))   options+=" Bottom-To-Top";
                if (getFlag(lFlags, CustomPlot.LEGEND_TRANSPARENT)) options+=" Transparent";
                Recorder.recordString("CustomPlot.addLegend(\""+labels+"\", \""+options+"\");\n");
            }
        }
    ...
    private void doHighResolutionDialog(Frame parent) {
        ...
        if (!gd.invalidNumber() && scale>0) //more range checking is done in CustomPlot.setScale
        ...
        if (Recorder.record) {
            ...
            if (options.length() > 0)
                options = ",\""+options+"\"";
            Recorder.recordString("CustomPlot.makeHighResolution(\""+title+"\","+hiResFactor+options+");\n");
        }
    }
