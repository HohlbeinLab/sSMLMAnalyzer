%% Wrapper for spectral_pairFinding based on ThunderSTORM-pSMLM input
%Default values:
% angrange = [-0.04*pi 0*pi];
% distrange = [2500 4500];

function [particles, allpossibilities] = Analysis_data_spectrum_ThunderSTORM_pSMLM(inputThSTORMcsv,angrange,distrange,savecsv,csvloc)
ThSTORMdata = csvread(inputThSTORMcsv,1,0);
frxylist = ThSTORMdata(:,[2 3 4 5 8 7 6 9 10]); %phasor THSTORM
%Frxylist is now frame	x [nm]	y [nm]	z [nm]	sigma1 [nm]	sigma2 [nm]	intensity [photon]	offset [photon]	bkgstd [photon]	uncertainty_xy [nm]

[particles, allpossibilities] = spectral_pairFinding_pSMLM(frxylist,angrange,distrange);
