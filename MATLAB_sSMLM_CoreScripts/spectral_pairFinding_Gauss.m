...
% ThunderSTORM-Gauss fitting
function [particles, allpossibilities] = spectral_pairFinding_Gauss(loc_list,curveAngleinput,linking_range)
%%
...
%                                 check if it will be found
    ...
    for k = 1:size(idlist,1)
        ...
        particlesT(k,7) = all_array(idlist(k,1),5); %sigma1 of spatial
        particlesT(k,8) = all_array(idlist(k,1),6); %sigma2 of spatial
        particlesT(k,9) = all_array(idlist(k,2),5); %sigma1 of spectral
        particlesT(k,10) = all_array(idlist(k,2),6); %sigma2 of spectral
        particlesT(k,11) = all_array(idlist(k,1),9); %locuncertainty of spatial
